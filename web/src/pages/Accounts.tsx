...
;
  ...
  const fetchAccounts = async (silent = false) => {
    ...
    try {
      ...
      setAccounts(data);
      // 并行查询每个账号的积分（套餐积分 usageItems[0]）
      const pmap: Record<string, { used: number; remain: number; total: number }> = {};
      await Promise.all(data.map(async (acc) => {
        try {
          const p = await api.getAccountPoint(acc.user_id);
          const item = Array.isArray(p?.usageItems) && p.usageItems.length > 0
            ? p.usageItems.find((i: any) => i.type === 'seatPackage') || p.usageItems[0]
            : null;
          if (item) {
            pmap[acc.user_id] = {
              used: item.used ?? 0,
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 110,
      render: (_: unknown, record: Account) => {
        const p = points[record.user_id];
        return (
          <div style={{ lineHeight: 1.4 }}>
            <Typography.Text strong style={{ fontSize: 13 }}>{p ? p.used : '-'}</Typography.Text>
            <br />
            <Typography.Text type="secondary" style={{ fontSize: 11 }}>总量 {p ? p.total : '-'}</Typography.Text>
          </div>
        );
      },
    },
    {
      ...
      render: (_: unknown, record: Account) => {
        const p = points[record.user_id];
        const pct = p && p.total > 0 ? Math.round((p.used / p.total) * 100) : 0;
        return (
          <div style={{ lineHeight: 1.4 }}>
            <Typography.Text strong style={{ fontSize: 13 }}>{p ? p.remain : '-'}</Typography.Text>
            <br />
            <Typography.Text type="secondary" style={{ fontSize: 11 }}>{p ? `已用 ${pct}%` : ''}</Typography.Text>
          </div>
        );
      },
    },
      ...
      >
        ...
        >
          <Table
            ...
            locale={{
              emptyText: (
                <div style={{ padding: '28px 16px', color: '#64748b', lineHeight: 1.8, maxWidth: 460, margin: '0 auto', whiteSpace: 'normal' }}>
                  暂无账号，请点击「一键导入」或「OAuth授权登录」配置第一个 JoyCode 账号
                </div>
              ),
            }}
          />
      ...
      >
        {oauthWaiting && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, color: '#666' }}>
            <Spin />
            <span style={{ fontSize: 13 }}>已打开授权页面，正在自动检测授权结果（本地部署适用；NAS/远程部署请改用「扫码登录」自动添加）...</span>
          </div>
        )}
        {oauthFallback && (
          <>
            <Alert
              ...
              showIcon
              message="完成授权后"
              description="本地部署会自动检测并添加账号。若使用 Docker 或远程部署（如 NAS），浏览器会跳转到一个无法访问的 localhost 页面——这是正常现象。推荐改用「扫码登录」：手机端 JoyCode 扫码即可自动添加账号。如需继续本方式：复制该页面地址栏中的完整 URL 粘贴到下方，或直接粘贴 pt_key 后点击「提交授权」。"
