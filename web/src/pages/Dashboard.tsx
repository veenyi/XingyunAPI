...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="jc-page">
      ...
      {/* KPI 卡片 — 全宽网格 */}
      <div className="jc-kpi-grid" style={{ marginBottom: 16 }}>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={12}>
          <Card size="small" title={<span className="jc-section-title"><GlobalOutlined />账号积分分布（剩余）</span>}>
            {pointDistData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={pointDistData} layout="vertical" margin={{ left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_COLORS.grid} />
                  <XAxis type="number" tick={{ fontSize: 11, fill: CHART_COLORS.axis }} stroke={CHART_COLORS.grid} />
                  <YAxis dataKey="name" type="category" width={90} tick={{ fontSize: 11, fill: CHART_COLORS.axis }} stroke={CHART_COLORS.grid} />
                  <Tooltip
                    contentStyle={{ background: 'var(--jc-bg-elevated)', border: '1px solid #334155', borderRadius: 8, fontSize: 12 }}
                    itemStyle={{ color: 'var(--jc-fg)' }}
                    labelStyle={{ color: 'var(--jc-fg-muted)' }}
              ...
              <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--jc-fg-muted)', fontSize: 13 }}>
                暂无积分数据，请先在账号管理中添加账号
              </div>
            )}
          </Card>
            ...
            <Row gutter={[8, 12]}>
              ...
              <Col span={24}>
                ...
                <Statistic
                  ...
                  valueStyle={{ fontSize: 18, color: lowPointCount > 0 ? CHART_COLORS.warning : CHART_COLORS.primary }}
                />
                <Typography.Text type="secondary" style={{ fontSize: 11 }}>
                  聚合 Key 会自动跳过低积分账号，切换到剩余充足的账号
                </Typography.Text>
              </Col>
              <Col span={12}>
